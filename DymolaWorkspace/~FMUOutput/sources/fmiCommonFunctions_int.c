...
DYMOLA_STATIC FMIStatus fmi3Get(FMIComponent c, typePointer *tp, fmi3Types type, const FMIValueReference* vr, size_t nvr, size_t nval){
	...
	for (i = 0; i < nvr; i++) {
		...
		if (0xffffffff == r) {
			...
			ix = 0xfffffffe - r;
			value = comp->crossingFunctions[ix];
		}else{
			switch (iCat) {
			case dsOutput:
				value = comp->outputs[ix];
				break;
			case dsAux:
				value = comp->auxiliary[ix];
				break;
			case dsState:
				value = comp->states[ix];
				break;
			case dsDerivative:
				value = comp->derivatives[ix];
				break;
			case dsParameter2:
			case dsParameter:
				value = comp->parameters[ix];
				break;
			case dsInput: 
				value = comp->inputs[ix];
				break;
			case 11: /*dState */
				value = comp->dStates[ix];
				break;
			case 12: /*PreviousVar*/
				value = comp->dStates[ix];
				break;
		...
		if(!isArray){
			...
			{
			case fmiTypeFloat64:
				tp->rp[i+arrayMod] = value;
				break;
			case fmiTypeInt32:
				tp->ip[i+arrayMod] = (fmi3Int32) value;
				break;
			case fmiTypeInt64:
				tp->i64p[i+arrayMod] = (fmi3Int64) value;
				break;
			case fmiTypeBoolean:
				tp->bp[i+arrayMod] = (fmi3Boolean) value;
				break;
			default:
				return FMIFatal;
			}
			util_logger(comp, comp->instanceName, FMIOK, loggFuncCall,
			"fmiGet%s: #%u# = %g",sfuncVer, r, value);
		}else{
			fmi3ValueReference arrayStartVr = arrayInternalValueReference[ix];
			size_t arraySize = arraySizes[ix];
			unsigned int aix = FMI_INDEX(arrayStartVr);
			unsigned int aiCat = FMI_CATEGORY(arrayStartVr);
			double * arrayStart = NULL;
			switch (aiCat) {
			case dsOutput:
				arrayStart = &comp->outputs[aix];
				break;
			case dsAux:
				arrayStart = &comp->auxiliary[aix];
				break;
			case dsState:
				arrayStart = &comp->states[aix];
				break;
			case dsDerivative:
				arrayStart = &comp->derivatives[aix];
				break;
			case dsParameter2:
			case dsParameter:
				arrayStart = &comp->parameters[aix];
				break;
			case dsInput: 
				arrayStart = &comp->inputs[aix];
				break;
			case 11: /*dState */
				arrayStart = &comp->dStates[aix];
				break;
			case 12: /*PreviousVar*/
				arrayStart = &comp->dStates[aix];
				break;
										...
										size_t nValues)
{
	...
	for ( i = 0; i < nValueReferences; i++) {
		const FMIValueReference r = valueReferences[i];
		int ix = FMI_INDEX(r);
		switch (FMI_CATEGORY(r)) {
		case dsStringParameter2:case dsStringParameter:
			values[i] = comp->sParameters[ix];
			break;
		default:
			util_logger(comp, comp->instanceName, FMIError, loggBadCall, 
...
DYMOLA_STATIC FMIStatus fmi3Set(FMIComponent c, ctypePointer *tp, fmi3Types type, const FMIValueReference* vr, size_t nvr, size_t nvals){
	...
	for (i = 0; i < nvr; i++) {
		...
		{
			if(iCat != dsInput2){ //Not array
				switch (type)
				{
				case fmiTypeFloat64:
					value = tp->rp[i+arrayMod];
					break;
				case fmiTypeInt32:
					value = (FMIReal)tp->ip[i+arrayMod];
					break;
				case fmiTypeInt64:
					value = (FMIReal)tp->i64p[i+arrayMod];
					break;
				case fmiTypeBoolean:
					value = (FMIReal) tp->bp[i+arrayMod];
				break;
				default:
					/*illegal type implementation error*/
					return FMIFatal;
					break;
				}
				switch (iCat) {
				case dsInput:
					if (comp->inputs[ix] != value) {
						comp->inputs[ix] = value;
						modified = FMITrue;
						if (comp->isCoSim) {
				...
				case dsAux:
					...
					if (comp->mStatus == modelInstantiated || comp->mStatus == modelInitializationMode || comp->AdvancedDiscreteStates) {
						CHECK_MODIFIED(comp->auxiliary[ix])
					}else {
						allowed = FMIFalse;
					}
					break;	
				case dsParameter:
					if (comp->parameters[ix] != value) {
						comp->parameters[ix] = value;
						modified = FMITrue;
				...
				case dsDerivative:
					if (comp->mStatus == modelInstantiated || comp->mStatus == modelInitializationMode){
						CHECK_MODIFIED(comp->derivatives[ix])
					} else {
						allowed = FMIFalse;
					}
					break;
				case dsOutput:
					if (comp->mStatus == modelInstantiated || comp->mStatus == modelInitializationMode){
						CHECK_MODIFIED(comp->outputs[ix])
					} else {
						allowed = FMIFalse;
					}
					break;
				case dsDState:
					CHECK_MODIFIED(comp->dStates[ix])
					break;
				case dsPrevious: 
					CHECK_MODIFIED(comp->dStates[ix])/*Set the state is correct*/
					break;
				case dsParameter2:
				case dsInput2:
					allowed = FMIFalse;
				...
				default:
					util_logger(comp, comp->instanceName, FMIError, loggBadCall,
						"fmiSet%s: cannot set #%u#  ",sfuncVer, r);
					return util_error(comp);
				}
			}else{ /*Array*/
				fmi3ValueReference arrayStartVr = arrayInternalValueReference[ix];
				...
				if(allowed){
					if(type == fmiTypeFloat64){
						...
						for(j=0; j< arraySize; ++j){
							...
							{
							case fmiTypeInt32:
								arrayStart[j] = (double) tp->ip[i+arrayMod+j];
								break;
							case fmiTypeInt64:
								arrayStart[j] = (double) tp->i64p[i+arrayMod+j];
								break;
							case fmiTypeBoolean:
								 arrayStart[j] = (double) tp->bp[i+arrayMod+j];
								break;
							default:
								return FMIFatal;
							}
...
DYMOLA_STATIC FMIStatus fmi3SetString_(FMIComponent c, const FMIValueReference vr[], size_t nvr, const FMIString  value[], size_t nvals)
{
	...
	for (i = 0; i < nvr; i++) {
		...
		if (allowed == FMIFalse) {
			util_logger(comp, comp->instanceName, FMIError, loggBadCall,
				"fmi3Set%s: may not change #%u# at this stage",label, r);
			return util_error(comp);
		}
		util_logger(comp, comp->instanceName, FMIOK, loggFuncCall,
...
DYMOLA_STATIC FMIStatus fmiGetFMUstate_(FMIComponent c, FMIFMUstate* FMUstate)
{
	...
	if (source->isCoSim && !source->istruct->mInlineIntegration) {
		...
		if (status == FMIOK) {
			...
			util_logger(c, source->instanceName, FMIError, loggUndef, "%s failed",label);
			return util_error(source);
		}
	}
#endif
											  ...
											  size_t size)
{
	...
	EXCEPTION_TRY
		/* setup tpl */
		tn = createTplMap((Component*)FMUstate, comp->tplDidBin);
		if (tn == NULL) {
			status = FMIError;
			goto clean;
		}
...
#if (FMI_VERSION >= FMI_3)
	if(nSeed != nv || nSensitivity != nz){
		util_logger(comp, comp->instanceName, FMIError, loggBadCall, 
			"%s: nSeed (%u) and nSensitivity (%u) does not match with nKnowns (%u) and nUnknowns (%u)", label, nSeed, nSensitivity, nv, nz);
		return util_error(comp);
	}
#endif
...
done1:
	...
	/*  analytic Jacobian not available */
	{
		...
		FMIBoolean success2;
		/* avoid re-allocation on each call */
		if (DYNAllocateDirHelper(nv, nz, jacData)) {
			util_logger(comp, comp->instanceName, FMIError, loggMemErr, "%s: memory allocation failed", label);
			status = FMIError;
			goto done2;
		}
...
#if (FMI_VERSION >= FMI_3)
		if( (status = fmi3GetFloat64_(c, v_ref, nv, jacData->jacV, nv)) != FMIOK ||
			(status = fmi3GetFloat64_(c, z_ref, nz, jacData->jacZ, nz)) != FMIOK ){
#else
...
#if (FMI_VERSION >= FMI_3)
		if ((status = fmi3SetFloat64_(c, v_ref, nv, jacVTmp1,nv)) != FMIOK ) {
#else
		...
		comp->enforceRefresh = FMITrue;
#if (FMI_VERSION >= FMI_3)
		status = fmi3GetFloat64_(c, z_ref, nz, jacZTmp1,nz);
#else
...
#if (FMI_VERSION >= FMI_3)
		if ((status = fmi3SetFloat64_(c, v_ref, nv, jacVTmp2, nv)) != FMIOK ) {
#else
		...
		comp->enforceRefresh = FMITrue;
#if (FMI_VERSION >= FMI_3)
		status = fmi3GetFloat64_(c, z_ref, nz, jacZTmp2, nz);
#else
...
#if (FMI_VERSION >= FMI_3)
		}else if ((status = fmi3SetFloat64_(c, v_ref, 1, jacV, 1)) != FMIOK) {
#else
...
#if (FMI_VERSION >= FMI_3)
	if(nSeed != nUnknowns || nSensitivity != nKnowns){
		util_logger(comp, comp->instanceName, FMIError, loggBadCall, 
			"%s: nSeed (%u) and nSensitivity (%u) does not match with nUnknowns (%u) and nKnowns (%u)", label, nSeed, nSensitivity, nUnknowns, nKnowns);
		return util_error(comp);
	}
#endif
	...
	if (QJacobianDefined_ && (comp->mStatus != modelInstantiated &&
		...
		for (j = 0; j < nSeed; j++) {
			...
			if (seed[j] != 0) {
				...
				int v_ix = FMI_INDEX(unknowns[j]);

				/* determine type for v reference*/
				dsCategory v_cat =(dsCategory) FMI_CATEGORY(unknowns[j]);
				switch(v_cat) {
				case dsDerivative:
					assert(v_ix < nx);
					break;
				...
				for (i = 0; i < nSensitivity; i++) {
					/* figure out which of A, B, C and D matrix to fetch from */
					dsCategory z_cat = (dsCategory) FMI_CATEGORY(knowns[i]);
					int z_ix = FMI_INDEX(knowns[i]);
					switch (z_cat) {
					case dsState:
						assert(z_ix < nx);
						if (isDerivative == FMITrue) {
							sensitivity[i] += seed[j] * jacData->jacA[z_ix * nx + v_ix ];
						} else {
							sensitivity[i] += seed[j] * jacData->jacC[z_ix * ny + v_ix ];
						}
						break;
					case dsInput:
						assert(z_ix < ni);
						if (isDerivative == FMITrue) {
							sensitivity[i] += seed[j] * jacData->jacB[z_ix * nx + v_ix];
						} else {
							sensitivity[i] += seed[j] * jacData->jacD[z_ix * ny + v_ix];
						}
						break;
					default:
						util_logger(comp, comp->instanceName, FMIError, loggBadCall,

...
DYMOLA_STATIC IntegrationStatus integration_setup(FMIComponent c, FMIReal relTol)
{
	...
	iData->sparseData.nnz = iData->sparseData.nGroups = iData->sparseData.nprocs = iData->sparseData.delayCVSuperLUMTCall = iData->sparseData.sparseEnabledVar = 0;
...

	{
		...
		if (nz)	{
			...
			iData->sparseData.delayCVSuperLUMTCall = 1;
		...
		if (0){
			...
			comp->iData->sparseData.nnz = iData->sparseData.delayCVSuperLUMTCall = 0;
...
DYMOLA_STATIC void updateIntegrationStatistics(Component* comp, Statistics* stat){
	...
	GET_STATISTIC(CVodeGetMaxOrder, maxOrder);
	stat->maxOrder = Dymola_max(maxOrder, stat->maxOrder);

#if !defined(FMU_SOURCE_CODE_EXPORT) || defined(FMU_SOURCE_CODE_EXPORT_SPARSE)
	if (comp->iData->sparseData.nnz) {
		if (!comp->iData->sparseData.delayCVSuperLUMTCall) {
...
DYMOLA_STATIC IntegrationStatus integration_reinit(Component* comp, int updateStatistics)
{
	...
	EXCEPTION_TRY
	if (comp->iData->sparseData.nnz)
		if(CVodeReInitSuperLUMT(comp->iData->cvode_mem) != CV_SUCCESS)
			return integrationError;
	EXCEPTION_CATCH_ALL
		return integrationFatal;
	EXCEPTION_END;
...
#if !defined(FMU_SOURCE_CODE_EXPORT) || defined(FMU_SOURCE_CODE_EXPORT_SPARSE)
DYMOLA_STATIC int PerformDelayedCvSuperLUMTCall(Component *comp)
{
	int flag;
	IntegrationData* iData = comp->iData;
	void* cvode_mem = iData->cvode_mem;
	...
	flag = CVSuperLUMT(cvode_mem, iData->sparseData.nprocs, (int)comp->nStates, iData->sparseData.nnz);
	if (util_check_flag(&flag, "CVSuperLUMT", 1, comp)) goto fail;

	/* use own Jacobian function to enable use of grouping */
	CALL_SET(CVSlsSetSparseJacFn, jac_JacobianSparse);

	iData->sparseData.delayCVSuperLUMTCall = 0;
...
static IntegrationStatus clone_data(Component* target, Component* source)
{
	...
	iData->sparseData.nprocs = source->iData->sparseData.nprocs;
	iData->sparseData.delayCVSuperLUMTCall = source->iData->sparseData.delayCVSuperLUMTCall;
	iData->sparseData.sparseEnabledVar = source->iData->sparseData.sparseEnabledVar;
...
static IntegrationStatus free_data(Component* comp, Component* target)
{
	...
	{ 
		...
		DYNPropagateDidToThread(comp->did);
		EXCEPTION_TRY
		if (!iData->sparseData.delayCVSuperLUMTCall)
			CVodeFreeSuperLUMTGlobalData(iData->cvode_mem);
		EXCEPTION_CATCH_ALL
			return integrationFatal;
		EXCEPTION_END;
#endif
